...
`;
  ...
  function walkStatuses(obj,path,out){
    ...
    if(Object.prototype.hasOwnProperty.call(obj,'status')){
      var st=obj.status,base={path:path,label:humanPath(path),status:st,notes:obj.notes||obj.observacao||'',occurrenceId:obj.occurrenceId||obj.ocorrenciaId||obj.idOcorrencia||'',legalRef:obj.legalRef||obj.dispositivo||obj.referenciaLegal||''};
      if(st==='NC') out.nc.push(base);
      if(st==='NAO'||st==='ATUALIZAR') out.pending.push(base);
    }
    Object.keys(obj).forEach(function(k){if(k!=='status'&&k!=='photos')walkStatuses(obj[k],path?path+'.'+k:k,out);});
  }
  function dedupe(items){var seen={},out=[];(items||[]).forEach(function(x){var id=x.occurrenceId?('id:'+norm(x.occurrenceId)):('txt:'+norm(x.label)+'|'+norm(x.notes));if(seen[id])return;seen[id]=true;out.push(x);});return out;}
  function collectFinal(){var FM=window.FarmaciaManipulacao,state=FM.loadState(),out={nc:[],pending:[]};walkStatuses(state.sections||{},'sections',out);out.nc=dedupe(out.nc);out.pending=dedupe(out.pending);return out;}
  function finalKey(item){var s=(item.occurrenceId||item.path||item.label||'item').replace(/[^A-Za-z0-9_-]+/g,'_');return s.slice(0,160)||'item';}
  function finalField(path,label,rows,defaultValue){var FM=window.FarmaciaManipulacao,l=E('label','fm-field');l.appendChild(E('span','fm-field-label',label));var t=document.createElement('textarea');t.rows=rows||4;var current=FM.get(path,null);t.value=current==null?String(defaultValue==null?'':defaultValue):current;t.addEventListener('input',function(){FM.set(path,t.value,{source:'final-field'});});l.appendChild(t);return l;}
  function draftValue(path,fallback){var FM=window.FarmaciaManipulacao,v=FM.get(path,null);return v==null?String(fallback==null?'':fallback):String(v);}
  function classDescriptions(){return {
    'Satisfatório':'Não apresenta irregularidades que interfiram na atividade. Risco baixo. Decisão prevista: deferir/emissão da licença.',
    'Satisfatório com restrições':'Irregularidades não interferem diretamente na atividade nem na qualidade/segurança. Risco baixo. Pode haver comprovação documental/fotográfica e verificação posterior.',
    'Insatisfatório':'Irregularidades interferem na qualidade/segurança, mas são corrigíveis sem interrupção das atividades. Risco moderado. Prevê Auto de Infração, cronograma de adequação e reinspeção.',
    'Insatisfatório com Interdição Parcial':'Um ou mais setores/equipamentos comprometem a qualidade/segurança ou apresentam risco de acidentes. Risco alto. Prevê Auto de Infração e Termo de Interdição Parcial.',
    'Insatisfatório com Interdição Total':'As adequações não podem ser regularizadas sem interrupção das atividades. Risco alto. Prevê Auto de Infração, Termo de Interdição Total e indeferimento.'
  };}
  function reportText(){
    ...
    if(result.pending.length)result.pending.forEach(function(x,n){var key=finalKey(x),base='sections.final.pendingDrafts.'+key,pad=draftValue(base+'.pendencia',x.status==='NAO'?'Documento não apresentado':'Necessita atualização/complementação'),prazo=draftValue(base+'.prazo',''),obs=draftValue(base+'.observacao',x.notes);lines.push((n+1)+'. '+x.label+' — '+pad+(prazo?' — prazo: '+prazo:'')+(obs?' — '+obs:''));});else lines.push('Nenhuma pendência consolidada até o momento.');
    var comp=FM.get('sections.final.pendenciasComplementares','');if(comp)lines.push('Complementos: '+comp);
    lines.push('\nNÃO CONFORMIDADES');
    if(result.nc.length)result.nc.forEach(function(x,n){var key=finalKey(x),base='sections.final.ncDrafts.'+key,req=draftValue(base+'.requisito',x.label),constat=draftValue(base+'.constatacao',x.notes||'Constatação ainda sem anotação.'),disp=draftValue(base+'.dispositivo',x.legalRef||'');lines.push((n+1)+'. Requisito: '+req+' — Constatação: '+constat+(disp?' — Dispositivo: '+disp:''));});else lines.push('Nenhuma não conformidade marcada até o momento.');
    lines.push('\nCONSIDERAÇÕES FINAIS / AVALIAÇÃO DE RISCO\n'+(FM.get('sections.final.consideracoes','')||''));
    var cls=FM.get('sections.final.classificacao',''),desc=classDescriptions()[cls]||'';lines.push('\nCONCLUSÃO\n'+(cls||'Não selecionada')+(desc?' — '+desc:''));
  ...
  function renderFinal(target){var FM=window.FarmaciaManipulacao,result=collectFinal();target.innerHTML='';var wrap=E('section','fm-final-section');wrap.appendChild(E('h2','fm-section-title','Fechamento do relatório'));wrap.appendChild(E('p','fm-helper-text','A consolidação prepara minutas editáveis. Nenhuma foto, OCR, campo vazio ou anotação isolada cria não conformidade. A conclusão continua sendo escolha expressa da autoridade sanitária.'));
    var pend=E('section','fm-final-card');pend.appendChild(E('h3','', '10. Documentação Pendente'));if(result.pending.length){result.pending.forEach(function(x,n){var key=finalKey(x),base='sections.final.pendingDrafts.'+key,box=E('div','fm-final-draft');box.appendChild(E('strong','',(n+1)+'. '+x.label));var grid=E('div','fm-field-grid');grid.appendChild(finalField(base+'.pendencia','Pendência',2,x.status==='NAO'?'Documento não apresentado':'Necessita atualização/complementação'));grid.appendChild(finalField(base+'.prazo','Prazo',2,''));grid.appendChild(finalField(base+'.observacao','Observação',3,x.notes));box.appendChild(grid);pend.appendChild(box);});}else pend.appendChild(E('p','fm-helper-text','Nenhum documento pendente consolidado até o momento.'));pend.appendChild(E('p','fm-helper-text','“Apresentado com observação” não entra automaticamente como documentação pendente; a equipe define a necessidade de complementação.'));pend.appendChild(finalField('sections.final.pendenciasComplementares','Complementos de documentação não vinculados automaticamente aos itens acima',5,''));wrap.appendChild(pend);
    var nc=E('section','fm-final-card');nc.appendChild(E('h3','', '11. Não Conformidades'));if(result.nc.length){result.nc.forEach(function(x,n){var key=finalKey(x),base='sections.final.ncDrafts.'+key,box=E('div','fm-final-draft');box.appendChild(E('strong','',(n+1)+'. Minuta de não conformidade'));box.appendChild(finalField(base+'.requisito','Requisito negativado',2,x.label));box.appendChild(finalField(base+'.constatacao','Constatação',3,x.notes||'Constatação ainda sem anotação.'));box.appendChild(finalField(base+'.dispositivo','Legislação / dispositivo infringido',2,x.legalRef||''));nc.appendChild(box);});}else nc.appendChild(E('p','fm-helper-text','Nenhum item marcado como Não Conforme até o momento.'));nc.appendChild(E('p','fm-helper-text','Ocorrências com o mesmo identificador, ou duplicatas textuais exatas, são consolidadas para evitar repetição no relatório final.'));wrap.appendChild(nc);
    ...
    var cls=E('section','fm-final-card');cls.appendChild(E('h3','', '13. Conclusão'));cls.appendChild(E('p','fm-helper-text','A classificação é escolhida expressamente pela autoridade sanitária. O aplicativo não decide automaticamente.'));var descriptions=classDescriptions(),options=Object.keys(descriptions),current=FM.get('sections.final.classificacao',''),list=E('div','fm-classifications');options.forEach(function(v){var l=E('label'),r=document.createElement('input'),text=E('span','fm-classification-text');r.type='radio';r.name='fm-classificacao';r.value=v;r.checked=current===v;r.addEventListener('change',function(){if(r.checked)FM.set('sections.final.classificacao',v,{source:'final-classification'});});text.appendChild(E('strong','',v));text.appendChild(E('span','fm-helper-text',descriptions[v]));l.appendChild(r);l.appendChild(text);list.appendChild(l);});cls.appendChild(list);cls.appendChild(E('p','fm-helper-text','Para atividades sujeitas a AFE, conferir a necessidade de emissão da autorização antes do deferimento da licença. Nas situações insatisfatórias, registrar o cronograma de adequação e observar a suspensão/retomada do prazo administrativo conforme o manual aplicável.'));wrap.appendChild(cls);
