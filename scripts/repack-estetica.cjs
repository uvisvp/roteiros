'use strict';
/* Estética: injeta estetica-navegacao.js no fim do módulo
...
const START = '<!--ESTETICA_NAV_INICIO-->', END = '<!--ESTETICA_NAV_FIM-->';
const i0 = app.indexOf(START); if (i0 >= 0) app = app.slice(0, i0) + app.slice(app.indexOf(END) + END.length);
const code = fs.readFileSync(path.join(root, 'estetica-navegacao.js'), 'utf8').replace(/<\/(script)/gi, '<\\/$1');
...
console.log('Estética: navegação e inventário ajustados.');
