...
for(const[id,source]of base.blocks)if(!allowed.has(id))assert.equal(blocks.get(id),source,'Bloco protegido (banco normativo / citações) alterado em relação a '+baseRef+': '+id);
