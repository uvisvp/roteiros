'use strict';
/* Serviços assistenciais: injeta assistenciais-navegacao.js no fim do módulo
   (entre marcadores, idempotente). Uso: node scripts/repack-assistenciais.cjs */
...
const id = 'app--servicos-assistenciais';
...
const START = '<!--ASSIST_NAV_INICIO-->', END = '<!--ASSIST_NAV_FIM-->';
const i0 = app.indexOf(START); if (i0 >= 0) app = app.slice(0, i0) + app.slice(app.indexOf(END) + END.length);
const code = fs.readFileSync(path.join(root, 'assistenciais-navegacao.js'), 'utf8').replace(/<\/(script)/gi, '<\\/$1');
...
console.log('Serviços assistenciais: navegação simplificada injetada.');
