...
(function (window, document) {
  ...
  function parseAssay(raw,source,profile){
    ...
    return result('Relatório de Ensaio','relatorio-ensaio',{perfil:profile||'',relatorio:number,amostra:sample,codigoAmostra:code,lote:lot,fabricante:afterLabel(raw,'fabricante',250),coleta:collection,recebimento:receipt,emissao:issued,conclusao:conclusion,ensaios:tests},raw,source);
  }
  ...
  function applyResult(detail,fields,docRef){
    ...
    if(type==='relatorio-ensaio'){
      var ev=FM.get(dest,[]);if(!Array.isArray(ev))ev=[];ev.push({coleta:fields.coleta||'',certificados:fields.relatorio||'',amostra:fields.amostra||'',lote:fields.lote||'',fabricante:fields.fabricante||'',recebimento:fields.recebimento||'',emissao:fields.emissao||'',resultado:fields.conclusao||'',ensaios:fields.ensaios||'',codigoAmostra:fields.codigoAmostra||'',sourceDocument:docRef});FM.set(dest,ev,{source:'ocr-reviewed'});return;
    }
